					...
					while($arr = $dbRes->Fetch())
					{
						...
						foreach($arPropIds as $propId)
						{
							...
							if($arDefProps[$propId]['MULTIPLE'] && is_array($arr[$propId]))
							{
								...
								{
									$arCurElem[] = array_merge($arCurProp, array('VALUE'=>$v, 'DESCRIPTION'=>$arr['DESCRIPTION'][$propId][$k]));
								}
							}
							else
							{
								$arCurElem[] = array_merge($arCurProp, array('VALUE'=>$arr[$propId], 'DESCRIPTION'=>$arr['DESCRIPTION'][$propId]));
							}
						}
